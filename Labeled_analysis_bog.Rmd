...
* *Exchange Rate* columns from the *Labeled Compounds per File* table

```{r import_data, message=FALSE}
# Import data tables
compounds_file <- file.path(project_dir, 'data', 'Compounds.xlsx')
...
per_file_table <- per_file_table %>% 
  fill(Name, Formula, .direction = 'down') %>% 
  filter(str_detect(`# KEGG Pathways`, 'F')) %>% 
  filter(`# KEGG Pathways` != 'Study File ID')
per_file_table <- per_file_table[,1:13]
colnames(per_file_table) <- c('Name', 'Formula', 'Molecular Weight',	'RT [min] per file', 'FWHM [min]',	'Max. # MI',	'# Adducts',	'Area', 'Max. Exchange', 'Avg. Exchange', 'Rel. Exchange [%]', 'Status',	'Study File ID')
