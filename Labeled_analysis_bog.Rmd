...
#3. Data Manipularion and thermodynamic indices calculations
...
# Number of compounds per file
summary_table <- joint_table %>% 
  select(`Study File ID`, Name)

summary_table$Name <- ifelse(is.na(summary_table$Name), 'Not identified', 'Identified')
...
summary_table$`Study File ID` <- factor(summary_table$`Study File ID`, levels = c('F1', 'F2', 'F3', 'F4', 'F5', 'F6', 'F7', 'F8', 'F9', 'F10', 'F11', 'F12', 'F13', 'F14'))

summary_table$Status <- factor(summary_table$Status, levels = c('Not identified', 'Identified'))

sample_names <- tibble(`Study File ID` = c('F1', 'F2', 'F3', 'F4', 'F5', 'F6', 'F7', 'F8', 'F9', 'F10', 'F11', 'F12', 'F13', 'F14'),
                       sample_name = c('BNC', 'BNE', 'I_E1', 'I_E2', 'I_N1', 'I_N2', 'II_E1', 'II_E2', 'II_N1', 'II_N2', 'III_E1', 'III_E2', 'III_N1', 'III_N2'))

summary_table <- left_join(summary_table, sample_names, by = 'Study File ID')

id_summary_plot <- plot_col(summary_table, sample_name, n, Status, `Sample Type`) +
